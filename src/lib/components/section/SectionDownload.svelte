...
<Section title={section.name?.value ?? ''} level={2} class={className}>
  ...
  {:else if section.filename?.value}
    <div
      class="leading-1.5 flex w-full max-w-[326px] flex-col rounded-e-xl rounded-es-xl border-gray-200 bg-gray-100 p-4 dark:bg-gray-700"
    >
      <div class="my-2.5 flex items-start rounded-xl bg-gray-50 p-2 dark:bg-gray-600">
        <div class="me-2">
          <span
