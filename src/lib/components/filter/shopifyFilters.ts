  ...
  const values = filter.values.map((v) => {
    ...
    if (filterType === 'PRICE_RANGE') {
      ...
      return {
        ...
        key,
        value,
        active: searchValues.some((v) => v.endsWith(value)),
      }
    }
  })
  return {
...
export function getCategoryFilterAsTree(
  ...
  const parentByChildId = new Map(menuItems.map((h) => [shortGID(h.id), shortGID(h.parent?.id)]))
  const includedValues = filter.values.filter((v) =>
    menuItems.some((i) => shortGID(i.id) === v.value),
  )

  const itemsById = new Map(
    includedValues.map((v) => [v.value, { ...v, children: [] as EnhancedFilterItem[] }]),
  )
...
export function getActiveShopifyFilters(
  filters: Filter[],
  menuStructure: MenuItem | undefined,
  searchParams: URLSearchParams,
): ProductFilter[] {
  const parentIdByChildId = new Map(
    flatten(menuStructure).map(({ id, parent }) => [shortGID(id), shortGID(parent?.id)]),
  )
  ...
  return filters
    ...
    .flatMap((f) =>
      f.values
        ...
        .filter(
          (v, _i, arr) =>
            f.key !== 'category' || !arr.some((a) => parentIdByChildId.get(a.value) === v.value),
        )
        .flatMap(({ input }) => {
          const parsed = JSON.parse(input) as ProductFilter
