<script lang="ts">
  ...
  import type { TGenericFile } from './DownloadButton.gql'

  export let file: TGenericFile
  export let text: string
</script>

<div class="my-2.5 flex w-full flex-col items-start gap-6 rounded-xl bg-accent p-6">
  <div class="me-2 text-muted-foreground">
    ...
    <span class="flex gap-2 text-xs font-normal">
      {#if file?.originalFileSize}
        {Math.round(file?.originalFileSize / 1024)} KB
      {:else}
        Okänd filstorlek
      {/if}
      <svg
